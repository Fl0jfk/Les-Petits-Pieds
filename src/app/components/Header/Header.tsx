...
export default function Header(){
    ...
    return (
        <motion.header  variants={{ visible: { y: 0 }, hidden: { y: "-100%" }}}  animate={hidden ? "hidden" : "visible"} transition={{duration: 0.35, ease: "easeInOut"}} className={`flex p-4 justify-between items-center w-full sm:fixed md:fixed z-[12] md:mb-[100px] ${opacityMenu} self-center text-2xl overflow-hidden max-w-[1500px] mx-auto bg-[#FFF5E4]`}>
                <div className='w-2/12 flex items-center h-full sm:w-4/12 md:w-4/12'>
                    {Logo &&
                        <Link href="/">
                            <Image src={Logo} alt='Mon memoji' width={130} height={130} className={`cursor-pointer z-[8] ${opacityLogo}`} quality={100} onClick={()=>{window.scrollTo({top:0, left:0, behavior:'smooth'})}}/>
                        </Link>
                    }
                </div>
